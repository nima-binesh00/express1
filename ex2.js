...
app.get("/:category/:subcategory/:id", (req, res) => {
  ...
  const foundCategory = storeData.find((item) => item.category === category);
  if (!foundCategory) {
    return res.status(404).json({ message: "Did not find the category" });
  }
  const foundSubcategory = foundCategory.subcategories.find(
    (sub) => sub.subcategory === subcategory
  );
  if (!foundSubcategory) {
    return res.status(404).json({ message: "Did not find the subcategory" });
  }
  const foundProduct = foundSubcategory.products.find(
    (product) => product.id === +id
  );
  if (!foundProduct) {
    return res.status(404).json({ message: "Did not find the product" });
  }
